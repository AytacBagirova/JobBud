...
useEffect(() => {
  if(wallet) 
  dispatch(getWalletHistory(wallet.id))
}, [walletState])

  const totalBalanceCard = (columnOfCard) => {
    return <div class={`col-md-${columnOfCard}`}>
  ...
  const pendingAmountCard = (columnOfCard) => { 
     return  <div class={`col-md-${columnOfCard}`}> <div class="card bg-primary text-light" style={{ height: 120 }}>
          <div class="card-body">
            <div class="mini-stat clearfix">
              <span class="mini-stat-icon bg-brown me-0 float-end">
                <i class="fa-solid fa-user"></i>
  ...
  totalBalanceCard(8)
    ...
    {pendingAmountCard(5)}
      ...
      <div className="payments-container">
        {loadingHistory ? "Loading please wait." :
          historyError ? (<div className="alert alert-danger my-2" role="alert">{error}</div>) : walletHistory ?
          ...
          <div key={transaction.id} className="payment-card">
            ...
            <div className="payment-amounts">
              ...
              <p className="text-success">{transaction.actionType}</p>
            </div>
